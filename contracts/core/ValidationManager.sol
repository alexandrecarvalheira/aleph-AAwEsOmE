...
import {IValidator, IModule, IPolicy, IExecutor, IHook, ISigner, IFallback} from "../interfaces/IERC7579Modules.sol";
...
abstract contract ValidationManager is EIP712, SelectorManager, HookManager, ExecutorManager {
    ...
    function _doValidation(ValidationMode vMode, ValidationId vId, Transaction calldata op, bytes32 userOpHash)
        internal
        returns (ValidationData validationData)
    {
        ValidationStorage storage state = _validationStorage();
        Transaction memory userOp = op;
    ...
    function _checkUserOpPolicy(PermissionId pId, Transaction memory userOp, bytes calldata userOpSig)
        internal
